...
const page = () => {
  ...
  const HandleEdit = (id:number | undefined) => {
    router.push(`other-devices/Form/${id}`)
  }
  ...
  const fetchData = async () => {
    try {
      const respons = await axios.get("/api/otherDevice");
  ...
  return (
    <div className="px-4 py-4">
      {Toggle && (
        <div
          className="bg-gray-300/70 backdrop-blur-[1px]  z-40 w-9/12  h-140  absolute  px-10 py-30 "
      ...
      <div className="flex w-12/12 items-center justify-between border-b-[0.5px] pb-4 border-gray-400">
        <div className="flex flex-col space-y-2">
          <h1 className="text-2xl font-bold">Other-devices</h1>
          <p className="text-sm text-gray-500">
            Manage of all Devices asset in your organization
          </p>
        </div>
      </div>
        ...
        <Table className="bg-white shadow-md shadow-gray-300 rounded-md">
          ...
          <TableHeader>
            ...
            <TableRow className="py-4 bg-gray-50">
              ...
              <TableHead className=" px-3 text-right font-semibold">
                Actions
              </TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {PC_info.map((pc) => (
              <TableRow key={pc.id}>
                <TableCell>{pc.device_name}</TableCell>
                <TableCell>{pc.device_type}</TableCell>
                <TableCell>{pc.functions}</TableCell>
